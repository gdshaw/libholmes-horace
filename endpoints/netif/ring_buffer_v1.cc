...
ring_buffer_v1::ring_buffer_v1(packet_record_builder& builder, size_t snaplen,
	...
	// frames.
	_tpreq.tp_frame_size = TPACKET_ALIGN(TPACKET_HDRLEN + ETH_HLEN) +
...
const record& ring_buffer_v1::read() {
	...
	volatile struct tpacket_hdr* tphdr = (struct tpacket_hdr*)frame_ptr;
...
const std::string& ring_buffer_v1::method() const {
	static const std::string name("ringv1");
	return name;
}
