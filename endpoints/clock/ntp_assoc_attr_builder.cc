...
void ntp_assoc_attr_builder::add_associd(uint16_t associd) {
	_attrs.insert(_associd_attr = unsigned_integer_attribute(_associd_attr.attrid(), associd));
}

void ntp_assoc_attr_builder::add_status(uint16_t status) {
	_attrs.insert(_status_attr = unsigned_integer_attribute(_status_attr.attrid(), status));
}

void ntp_assoc_attr_builder::add_srchost(const std::string& srchost) {
	_attrs.insert(_srchost_attr = string_attribute(_srchost_attr.attrid(), srchost));
}

void ntp_assoc_attr_builder::add_srcaddr(const std::string& srcaddr) {
	_attrs.insert(_srcaddr_attr = string_attribute(_srcaddr_attr.attrid(), srcaddr));
}

void ntp_assoc_attr_builder::add_srcport(uint16_t srcport) {
	_attrs.insert(_srcport_attr = unsigned_integer_attribute(_srcport_attr.attrid(), srcport));
}

void ntp_assoc_attr_builder::add_stratum(unsigned int stratum) {
	_attrs.insert(_stratum_attr = unsigned_integer_attribute(_stratum_attr.attrid(), stratum));
}

void ntp_assoc_attr_builder::add_delay(int64_t delay) {
	_attrs.insert(_delay_attr = signed_integer_attribute(_delay_attr.attrid(), delay));
}

void ntp_assoc_attr_builder::add_offset(int64_t offset) {
	_attrs.insert(_offset_attr = signed_integer_attribute(_offset_attr.attrid(), offset));
}

void ntp_assoc_attr_builder::add_jitter(int64_t jitter) {
	_attrs.insert(_jitter_attr = signed_integer_attribute(_jitter_attr.attrid(), jitter));
}
